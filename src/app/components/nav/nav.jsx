      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-3 bg-transparent mt-8 sm:mt-11 overflow-hidden">
        <div className="flex items-center justify-between max-w-[1400px] mx-auto">
          <div className="flex items-center sm:gap-4">
            ...
            {logoData?.data?.logo && (
              <Image src={logoData.data.logo} alt="Logo" width={122} height={50} />
            )}
            <CategoriesMenu />
          </div>
          ...
          <div className="flex gap-4 mt-2 items-center">
            <Link href="#" className="flex gap-1 hover:text-purple-600 transition-colors">
              EN <SlGlobe className="text-lg" />
            </Link>
            <button className="flex gap-1 hover:text-purple-600 transition-colors">
              <FaSearch />
            </button>
            <Link href="/login" className="text-black hover:text-purple-600 transition-colors hidden lg:flex">
              تسجيل دخول
            </Link>
        ...
        <nav className="w-full px-4 sm:px-6 lg:px-8 py-3 bg-transparent mt-8 sm:mt-11">
          <div className="flex items-center justify-between max-w-[1400px] mx-auto">
            ...
            <div className="flex gap-4 mt-2 items-center">
              ...
              {isLoggedIn ? (
                <UserProfileSection userData={userData} isLoading={isLoading} isError={isError} />
              ) : (
                <Link
                  href="/login"
